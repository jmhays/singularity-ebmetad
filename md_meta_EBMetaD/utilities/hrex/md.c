/*
 * $Id: md.c,v 1.3 2010/07/29 15:32:00 gbussi Exp $
 * 
...
////////////////////////////////////////////////////////////////////////////////////
if(logical.meta_inp && logical.hrex) {
    gmx_enerdata_t *hrex_enerd;
    snew(hrex_enerd,1);
    init_enerdata(fplog,top_global->groups.grps[egcENER].nr,hrex_enerd);
/* first perform a fake exchange */
...
/* then compute the energy of the trial configuration */

      do_force(fplog,cr,ir,step,nrnb,wcycle,top,groups,
               state->box,state->x,&state->hist,
               f,buf,force_vir,mdatoms,hrex_enerd,fcd,
               state->lambda,graph,
               fr,vsite,mu_tot,t,fp_field,ed,
               GMX_FORCE_STATECHANGED | GMX_FORCE_NS |
               GMX_FORCE_ALLFORCES);
      colvar.hrex_energy=hrex_enerd->term[F_EPOT];
      plumed_sum(&mtd_data,1,&colvar.hrex_energy);

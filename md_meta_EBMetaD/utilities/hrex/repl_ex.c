/*
 * $Id: repl_ex.c,v 1.1 2010/05/04 13:13:18 gbussi Exp $
 * 
...
/* THIS IS A SPECIAL INITIALIZATION FOR PLUMED */
  if(logical.meta_inp){
      fprintf(fplog,"\nPLUMED IS ON: using a generalized replica exchange implementation\n\n");
      re->type = -1;
...
/* temperature */
      snew(re->alltemp,re->nrepl);
      re->alltemp[re->repl]=ir->opts.ref_t[0];
      gmx_sum_sim(re->nrepl,re->alltemp,ms);
...
/* lambda */
      ...
      if (ir->efep != efepNO) {
        re->bLAMBDA = TRUE;
        snew(re->alllambda,re->nrepl);
        re->alllambda[re->repl] = ir->init_lambda;
        gmx_sum_sim(re->nrepl,re->alllambda,ms);
      }

/* Index is just the processor number */
      snew(re->ind,re->nrepl);
      for(i=0; i<re->nrepl; i++)
        re->ind[i] = i;

/* log all the possible replica quantities */
      fprintf(fplog,"Repl   ");
      for(i=0; i<re->nrepl; i++)
        fprintf(fplog," %3d  ",re->ind[i]);
      fprintf(fplog,"\nRepl  T");
        for(i=0; i<re->nrepl; i++)fprintf(fplog," %5.1f",re->alltemp[re->ind[i]]);
      if (re->bNPT) {
        fprintf(fplog,"\nRepl  p");
        for(i=0; i<re->nrepl; i++)fprintf(fplog," %5.2f",re->pres[re->ind[i]]);
      }
      if (re->bLAMBDA) {
        fprintf(fplog,"\nRepl  l");
        for(i=0; i<re->nrepl; i++)fprintf(fplog," %5.3f",re->alllambda[re->ind[i]]);
      }
				...
				int step,real time)
{
  ...
  case -1: /*SPECIAL FOR PLUMED */
    ...
    snew(hrex_energy,re->nrepl);
    hrex_energy[re->repl]  = colvar.hrex_energy;
    gmx_sum_sim(re->nrepl,hrex_energy,ms);
...
/* metadynamics contribution */
        if(!logical.hrex){
          ...
          delta1 = Vbias[a]-Vbiasx[a] + Epot[a]-hrex_energy[a];
          delta2 = Vbias[b]-Vbiasx[b] + Epot[b]-hrex_energy[b];
        }
        betaA = 1.0/(re->alltemp[a]*BOLTZ);
        betaB = 1.0/(re->alltemp[b]*BOLTZ);
        delta_meta = - betaA * delta1 - betaB * delta2;
        
        delta = delta_meta;
        if(!logical.hrex){
/* these contribution are implicitly taken into account in hrex */
          delta+=(betaA - betaB)*(Epot[b] - Epot[a]);
          if (re->bLAMBDA)
            delta+=(re->alllambda[b] - re->alllambda[a])*(betaB*dvdl[a] - betaA*dvdl[b]);
        }
	break;
     case ereTEMP:
	/* Use equations from:
		      ...
		      int step,real time)
{
  ...
  if (bExchanged) {
	...
	scale_velocities(state,sqrt(re->q[ms->sim]/re->q[exchange]));
      if (re->type == -1)
        scale_velocities(state,sqrt(re->alltemp[ms->sim]/re->alltemp[exchange]));
    }
  }

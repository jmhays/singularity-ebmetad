...
#  sed -e "s/^AMBERBUILDFLAGS=/AMBERBUILDFLAGS=-DAMBER /" ${destination}/src/config_amber.h > tmp.h
  ...
  grep -q " -D__PLUMED_MPI" ${destination}/src/config.h &&
  {
    sed -e "s/^CPPFLAGS=/CPPFLAGS=-D__PLUMED_MPI /" ${destination}/src/config.h > tmp.h
...
function to_do_after_revert () {
  ...
  grep -q " -D__PLUMED_MPI" ${destination}/src/config.h &&
  {
    sed -e "s/^CPPFLAGS=-D__PLUMED_MPI /CPPFLAGS=/" ${destination}/src/config.h > tmp.h
    mv tmp.h ${destination}/src/config.h
  }
}

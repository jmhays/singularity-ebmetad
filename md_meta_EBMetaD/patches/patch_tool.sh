...
#-------------------
  ...
  if [ "$CAMSHIFT_LIBS" != "" ] ; then
    echo "-- Setting up camshift symlinks"
    for file in $CAMSHIFT_LINKS ; do
      base="${file##*/}"
      if test -e $WHERE_LINKS/$base ; then
        echo "PATCH ERROR: file $base is already in $WHERE_LINKS"
        exit 1
      fi
      if test -n "$CAMSHIFT_RENAME_RULE" ; then
        base="$($CAMSHIFT_RENAME_RULE $base)"
      fi
      ln -s $file $WHERE_LINKS/$base
    done
  fi
...
(-revert)
  ...
  if [ "$CAMSHIFT_LIBS" != "" ] ; then
    echo "-- Removing camshift symlinks"
    for file in $CAMSHIFT_LINKS ; do
      base="${file##*/}"
      if test -n "$CAMSHIFT_RENAME_RULE" ; then
        base="$($CAMSHIFT_RENAME_RULE $base)"

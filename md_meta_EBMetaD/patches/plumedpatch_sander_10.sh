...
function to_do_after_patch () {
  ...
  grep -q " -D__PLUMED_MPI" ${destination}/src/config_amber.h &&
  {
    sed -e "s/^CPPFLAGS=/CPPFLAGS=-D__PLUMED_MPI /" ${destination}/src/config_amber.h > tmp.h
    mv tmp.h ${destination}/src/config_amber.h
  }
  {
...
function to_do_after_revert () {
  ...
  grep -q " -D__PLUMED_MPI" ${destination}/src/config_amber.h &&
  {
    sed -e "s/^CPPFLAGS=-D__PLUMED_MPI /CPPFLAGS=/" ${destination}/src/config_amber.h > tmp.h
    mv tmp.h ${destination}/src/config_amber.h
  }
}

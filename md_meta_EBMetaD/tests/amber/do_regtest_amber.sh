...
#
#
dir_base=/Users/max/build/PLUMED/md_meta/tests/amber
maxtasks=1
amber_prefix="/Users/max/build/amber10/exe/sander "
...
dir_last=${dir_base}/LAST-amber
dir_out=${dir_base}/TEST-amber
...
do
 ...
 (
  ...
  do
     ...
     ( ${amber_prefix} -O -i ${input_file} -o global_info -p ../DIALA_TOP/system.prmtop -c ../DIALA_TOP/system.inpcrd -inf ${output_file} &>std_out  )
    # echo "EXECUTING ${amber_prefix} -O -i ${input_file} -o global_info -p system.prmtop -c system.inpcrd -inf ${output_file} &>std_out" 
     # *** amber failed obviously
     if (( $? )); then
     ...
     #    echo "SPECIAL TEST ${test_file[test_types]}" 
        ...
        # *** but didn't end !? (amber)
        grep -a "wallclock" global_info &> /dev/null
        if (( $? )); then
           echo "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" >> ${error_description_file}
           echo ${output_file} >> ${error_description_file}
           tail -40 ${output_file} >> ${error_description_file}
     ...
     "NEW" )
        ...
        timing=`grep -a "Total time" global_info | ${awk} '{printf("%6.2f",$4)}'`

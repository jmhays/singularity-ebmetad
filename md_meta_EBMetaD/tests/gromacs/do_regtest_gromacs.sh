...
#
# - dir_base: the base directory for testing (e.g. /Users/max/build/PLUMED/md_meta/tests/gromacs)
...
dir_base="/Users/carlo/Codes/md_meta/tests/gromacs"
maxtasks=1
gromacs_prefix="/Users/carlo/Codes/gromacs-4.5.5/bin/mdrun_mpi"
gromacs_version=4

# this can be used to set other mpirun commands (such as om-mpirun)
mpirun=mpiexec

# this is the mpi-compiled executable
gromacs_prefix_mpi="$gromacs_prefix"
...
dir_last=${dir_base}/LAST-gromacs
dir_out=${dir_base}/TEST-gromacs
...
function do_test_grep(){
 ...
 if [ "${grep_field}" == "0" ] ; then  #compare all the line
    ...
    e1=`grep -A1 -a "${grep_string}" ${output_old} | tail -1 | ${awk} -v f=${grep_field} '{f=-f;print $f}'`
    e2=`grep -A1 -a "${grep_string}" ${output_new} | tail -1 | ${awk} -v f=${grep_field} '{f=-f;print $f}'`
    big=`echo "${e1} ${e2}" | ${awk} '{v=sqrt((($1-$2)/$2)^2); if (v>1.0E-14) printf("%16.8e",v); else printf("0") ;}'`
 fi
 case ${big} in
 0)
 ...
 (
  ...
  if [[ ${noreset} != "noreset" ]]; then
     ...
     do
        ...
        rm -f ${dir_last}/${dir}/${reset_file}.log
     done
  fi
  #
  # run the tests now
  ...
  do
     ...
     output_file=${dir_out}/${dir}/${input_file}.log
     output_last=${dir_last}/${dir}/${input_file}.log

     # compare file: is the file through which you should compare

     if  [ "${test_file[test_types]}" == "std_out"  ] ; then 
         compare_file=${dir_out}/${dir}/${input_file}.log
         compare_last=${dir_last}/${dir}/${input_file}.log
         # Added for biasxmd and ptmetad
         if [ "${dir}" == "biasxmd" ] || [ "${dir}" == "ptmetad" ] || [ "${dir}" == "multiple" ] ; then
            compare_file=${dir_out}/${dir}/${input_file}0.log
            compare_last=${dir_last}/${dir}/${input_file}0.log
         fi
         #End added for biasxmd and ptmetad
     else
         compare_file=${dir_out}/${dir}/${test_file[test_types]}.log
         compare_last=${dir_last}/${dir}/${test_file[test_types]}.log #to conformate with reset syntax 
     fi
     ...
     case "${dir}" in
     (biasxmd | ptmetad)
       if [ ${gromacs_version} == 3 ] ; then
         ($mpirun -np 2  ${gromacs_prefix_mpi} -reseed 123 -plumed META_INP -np 2 -nice 0 -s ${input_file} -g ${output_file} -multi -replex 10 &>/dev/null )
       fi
      # for gromacs 4
       if [ ${gromacs_version} == 4 ] ; then
         ($mpirun -np 2  ${gromacs_prefix_mpi} -reseed 123 -plumed META_INP -nice 0 -s ${input_file} -g ${output_file} -multi 2 -replex 10 &>/dev/null )
       fi
       ;;
     (dd)
       # only for gromacs 4
        if [ ${gromacs_version} == 4 ] ; then
          ($mpirun -np 4  ${gromacs_prefix_mpi} -plumed plumed.dat -nice 0 -s ${input_file} -g ${output_file} &>/dev/null )
        fi
        ;;
     (pd)
        if [ ${gromacs_version} == 3 ] ; then
          ($mpirun -np 4  ${gromacs_prefix_mpi} -plumed plumed.dat -np 4 -nice 0 -s ${input_file} -g ${output_file} &>/dev/null )
        fi
       # for gromacs 4
        if [ ${gromacs_version} == 4 ] ; then
          ($mpirun -np 4  ${gromacs_prefix_mpi} -pd yes -plumed plumed.dat -nice 0 -s ${input_file} -g ${output_file} &>/dev/null )
        fi
        ;;
      (restart)
       cp HILLS.restart HILLS
       ( ${gromacs_prefix} -nice 0 -s ${input_file} -g  ${output_file} -plumed  &>/dev/null ) ;;
      (multiple)
       echo "cd w1/" > launch.sh
       echo "${gromacs_prefix} -nice 0 -s ${input_file} -g ${dir_out}/${dir}/${input_file}0 -plumed &>/dev/null &" >> launch.sh
       echo "cd ../w2" >> launch.sh
       echo "${gromacs_prefix} -nice 0 -s ${input_file} -g ${dir_out}/${dir}/${input_file}1 -plumed &>/dev/null"  >> launch.sh 
       echo "cd ../" >> launch.sh
       ( . launch.sh &>/dev/null ) ;;
      (*)
       ( ${gromacs_prefix} -nice 0 -s ${input_file} -g  ${output_file} -plumed  &>/dev/null ) ;;
     ...
     # Added for biasxmd and ptmetad and multiple

     if [ "${dir}" == "biasxmd" ]  || [ "${dir}" == "ptmetad" ] || [ "${dir}" == "multiple" ] ; then
        output_file=${dir_out}/${dir}/${input_file}0.log
        output_last=${dir_last}/${dir}/${input_file}0.log
     fi
     ...
     if (( $? )); then
        ...
        # *** but didn't end !? (namd2)
        grep -a "Finished mdrun" ${output_file} &> /dev/null
        if (( $? )); then
           echo "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" >> ${error_description_file}
           echo ${output_file} >> ${error_description_file}
           tail -40 ${output_file} >> ${error_description_file}
     ...
     "NEW" )
        ...
        # namd specific timing 
        timing=`grep -a "Time:" ${output_file} | ${awk} '{printf("%6.2f",$2)}'`

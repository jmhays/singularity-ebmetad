...
#
# - dir_base: the base directory for testing (e.g. /Users/max/build/PLUMED/md_meta/tests/namd) 
# - maxtasks: how many instances   should run simultaneously (~> #CPUs)
#
#
dir_base=/Users/chicco/Programs/gronamd/md_meta_devel/tests/namd
maxtasks=2
#namd_prefix="/Users/chicco/Programs/gronamd/md_meta_devel/codes/namd_2.7/NAMD_2.7b1_Source/MacOSX-x86-g++/namd2"
namd_prefix="/Users/chicco/Programs/gronamd/md_meta_devel/codes/namd_2.8/NAMD_2.8_Source/MacOSX-x86-g++/namd2"
...
dir_last=${dir_base}/LAST-namd
dir_out=${dir_base}/TEST-namd
 ...
 (
  ...
  do
     ...
     if  [ "${test_file[test_types]}" == "std_out"  ] ; then 
         compare_file=${dir_out}/${dir}/${input_file}.out
         compare_last=${dir_last}/${dir}/${input_file}.out
     else
     ...
     ( ${namd_prefix} ${input_file} &> ${output_file} )
     ...
     if (( $? )); then
        ...
        grep -a "Program finished." ${output_file} &> /dev/null
        if (( $? )); then
           echo "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" >> ${error_description_file}
           echo ${output_file} >> ${error_description_file}
           tail -40 ${output_file} >> ${error_description_file}
     ...
     "NEW" )
        ...
        timing=`grep -a "WallClock:" ${output_file} | ${awk} '{printf("%6.2f",$2)}'`

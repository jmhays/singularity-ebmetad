...
dir_base="/u/sbp/bussi/scratch/Plumed/cvs/tests/qespresso"
maxtasks=1
pw_prefix="/u/sbp/bussi/scratch/Plumed/all-codes/build/qespresso-4.3.2-serial/bin/pw.x -plumed"
cp_prefix="/u/sbp/bussi/scratch/Plumed/all-codes/build/qespresso-4.3.2-serial/bin/cp.x -plumed"

### ATTENTION:
# To run "cp" test, first you have to compute the starting (ground state) wavefunctions.
# Just go to cp/ directory, and launch "$cp_prefix < gs.in", where $cp_prefix is
# the path to cp.x executable. It will take a few minutes, and produce a directory named
# ch3cl_51.save
###
...
dir_last=${dir_base}/LAST-qespresso
dir_out=${dir_base}/TEST-qespresso
...
do
 ...
 (
  ...
  do
     ...
     case "${dir}" in
     (cp)
       ($cp_prefix < ${input_file} > ${output_file} 2>/dev/null ) ;;
     (pw)
       ($pw_prefix < ${input_file} > ${output_file} 2>/dev/null ) ;;
      esac

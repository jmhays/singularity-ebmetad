...
void PREFIX puckering_restraint(int i_c, struct mtd_data_s *mtd_data)
{
  ...
  real z[6];
  if(mtd_data->puckering_R==NULL) mtd_data->puckering_R=(rvec*) malloc(sizeof(rvec)*6);

  // generation of the geometrical center coordinate set R_j
  // and accessories vectors (see above)
  mod = generate_R(i_c,mtd_data,mtd_data->puckering_R,Rp,Rdp,RpxRdp);

  // generation of the projection z_j
  for(j=0;j<6;j++){
     z[j]=puckering_Zeta (mtd_data->puckering_R[j],RpxRdp,mod);
  }
  ...
  switch(colvar.type[i_c]) {
	case 1: // puckering coordinate Q
	     { 
                colvar.ss0[i_c] = Q;
		for(i=0;i<6;i++){
		  puckering_gradQ(gradQ, z, Q, mtd_data->puckering_R, Rp, Rdp, RpxRdp, mod, i); 
       ...
       case 2: // puckering coordinate phi
	    {
		    ...
		    colvar.ss0[i_c] = phi;
		    for(i=0;i<6;i++){
			puckering_gradphi(gradphi, z, mtd_data->puckering_R, Rp, Rdp, RpxRdp, mod, i);
       ...
       case 3: // puckering coordinate theta
	    {
		    ...
		    for(i=0;i<6;i++){
			puckering_gradtheta(gradtheta,z, mtd_data->puckering_R, Rp, Rdp, RpxRdp,mod,i);

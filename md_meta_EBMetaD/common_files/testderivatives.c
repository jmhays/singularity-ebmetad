  ...
  for(i_c=0;i_c<colvar.nconst;i_c++) {
...
#ifdef HAVE_ALMOST 
          case 60: camshift_restraint(i_c, mtd_data); break;
          case 61: camshiftens_restraint(i_c, mtd_data); break;
...
#endif
          case 65: phivalues_restraint(i_c, mtd_data); break;
          case 66: intracontact_restraint(i_c, mtd_data); break;
	...
	logical.not_same_step=0;
	  ...
	  case 53: msd_restraint(i_c, mtd_data); break;
          ...
          case 60: camshift_restraint(i_c, mtd_data); break;
          case 61: camshiftens_restraint(i_c, mtd_data); break;
          case 62: cs3meth_restraint(i_c, mtd_data); break;       
          case 63: cs3methens_restraint(i_c, mtd_data); break;    
          case 64: noe_restraint(i_c, mtd_data); break;              // CAMSHIFT ENERGY
#endif
          case 65: phivalues_restraint(i_c, mtd_data); break;
          case 66: intracontact_restraint(i_c, mtd_data); break;
...
void PREFIX debug_derivatives(struct mtd_data_s *mtd_data,int print){
  ...
  ncv = colvar.nconst;
  if(mtd_data->testderivatives_first){
    snew(mtd_data->testderivatives_integral,ncv);
    snew(mtd_data->testderivatives_pos,ncv);
    snew(mtd_data->testderivatives_der,ncv);
    snew(mtd_data->testderivatives_previous_pos,ncv);
    snew(mtd_data->testderivatives_previous_der,ncv);
    for(icv=0;icv<ncv;icv++){
      mtd_data->testderivatives_integral[icv]=colvar.ss0[icv];
      snew(mtd_data->testderivatives_pos[icv],colvar.natoms[icv]);
      snew(mtd_data->testderivatives_der[icv],colvar.natoms[icv]);
      snew(mtd_data->testderivatives_previous_pos[icv],colvar.natoms[icv]);
      snew(mtd_data->testderivatives_previous_der[icv],colvar.natoms[icv]);
    }
  }
  ...
  for(icv=0;icv<ncv;icv++){
    for(i=0;i<colvar.natoms[icv];i++){
      ...
      for(j=0;j<3;j++){
        mtd_data->testderivatives_der[icv][i][j]=colvar.myder[icv][i][j];
        mtd_data->testderivatives_pos[icv][i][j]=mtd_data->pos[iat][j];
      }
    }
  }
  ...
  if(!mtd_data->testderivatives_first){
    for(icv=0;icv<ncv;icv++){
      for(i=0;i<colvar.natoms[icv];i++){
        for(j=0;j<3;j++){
          mtd_data->testderivatives_integral[icv]+=
          0.5*(mtd_data->testderivatives_der[icv][i][j]+mtd_data->testderivatives_previous_der[icv][i][j])*(mtd_data->testderivatives_pos[icv][i][j]-mtd_data->testderivatives_previous_pos[icv][i][j]);
        }
      }
    }
  }
  ...
  if(print){
    if(!mtd_data->testderivatives_debug_file) mtd_data->testderivatives_debug_file = fopen((mtd_data->ionode?"DEBUG":"/dev/null"), "w");
    fprintf(mtd_data->testderivatives_debug_file, "%10.3f", mtd_data->time);
    for(icv=0;icv<colvar.nconst;icv++) fprintf(mtd_data->testderivatives_debug_file, "   %10.8f", mtd_data->testderivatives_integral[icv]-colvar.ss0[icv]);
    fprintf(mtd_data->testderivatives_debug_file, "\n");
    fflush(mtd_data->testderivatives_debug_file);
  }

  for(icv=0;icv<ncv;icv++){
    for(i=0;i<colvar.natoms[icv];i++){
      for(j=0;j<3;j++){
        mtd_data->testderivatives_previous_der[icv][i][j]=mtd_data->testderivatives_der[icv][i][j];
        mtd_data->testderivatives_previous_pos[icv][i][j]=mtd_data->testderivatives_pos[icv][i][j];
      }
    }
  }

  mtd_data->testderivatives_first=0;
};

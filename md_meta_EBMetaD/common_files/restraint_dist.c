...
void PREFIX dist_restraint(int i_c, struct mtd_data_s *mtd_data)
{
  ...
  if(colvar.averaging[i_c]) {
    if(logical.remd) { 
      if(mtd_data->repl!=-1) { 
        ...
        plumed_error("IN ORDER TO USE AVERAGING YOU MUST RUN GROMACS WITH MULTIPLE REPLICAS AND BIASXMD");
      }
    } else {
      plumed_error("IN ORDER TO USE AVERAGING YOU MUST RUN GROMACS WITH MULTIPLE REPLICAS AND BIASXMD");
    }
    if(firstTime) fprintf(mtd_data->fplog, "|- DISTANCE AVERAGED OVER %i REPL WITH FACT %lf\n", repliche, fact); fflush(mtd_data->fplog);
  }
  ...
  if(colvar.averaging[i_c]) {
    ...
    if(MASTER(mtd_data->mcr)) {
      // among replicas 
      plumed_intersum(mtd_data, 1, &mod_rij); 
      mod_rij *= fact; 
    } else mod_rij = 0.;
    // inside each replica
    plumed_sum(mtd_data, 1, &mod_rij);
    /* end */
  }
#endif
  ...
  for(i=0;i<colvar.list[i_c][0];i++) { 
    iat = colvar.cvatoms[i_c][i];
    colvar.myder[i_c][i][0] =  fact*(mtd_data->mass[iat]*rij[0])/(mass1*mod_rij);
    colvar.myder[i_c][i][1] =  fact*(mtd_data->mass[iat]*rij[1])/(mass1*mod_rij);
    colvar.myder[i_c][i][2] =  fact*(mtd_data->mass[iat]*rij[2])/(mass1*mod_rij);
  }
  for(i=colvar.list[i_c][0];i<colvar.natoms[i_c];i++) {
    iat = colvar.cvatoms[i_c][i];
    colvar.myder[i_c][i][0] = -fact*(mtd_data->mass[iat]*rij[0])/(mass2*mod_rij);
    colvar.myder[i_c][i][1] = -fact*(mtd_data->mass[iat]*rij[1])/(mass2*mod_rij);
    colvar.myder[i_c][i][2] = -fact*(mtd_data->mass[iat]*rij[2])/(mass2*mod_rij);
  }
...
int PREFIX read_dist(char **word, int count, t_plumed_input *input, FILE *fplog)
{
  ...
  iw=seek_word(word,"AVERAGING");
  if(iw>=0) {
#ifdef __PLUMED_MPI
  colvar.averaging[count] = 1;}
#else
  plumed_error("AVERAGING CANNOT WORK WITHOUT MPI!");}
#endif

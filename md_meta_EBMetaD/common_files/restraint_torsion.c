...
void PREFIX torsion_restraint(int i_c, struct mtd_data_s *mtd_data)
{
  ...
  if( colvar.doTrig[i_c]==0 ){ 
     ...
     real newpsi;
     newpsi=psi+offset; 
     if (newpsi >= pi) newpsi -= twopi;
     else if(newpsi < -pi) newpsi += twopi;	
     cos_psi=cos( newpsi );
     for(i=0;i<3;++i){d[0][i]*= cos_psi; d[3][i]*= cos_psi; d[1][i]*= cos_psi; d[2][i]*= cos_psi;}
     colvar.ss0[i_c] = sin( newpsi );
  }
...
int PREFIX read_torsion(char **word, int count, t_plumed_input *input, FILE *fplog)
{
  ...
  iw=seek_word(word,"OFFSET");
  if(iw>=0){ sscanf(word[iw+1],"%lf", &delta); 
	     colvar.offset[count]=(real) delta; }else{ colvar.offset[count]=(real) 0.; };
  ...
  if (colvar.offset[count]<-1.e-7 || colvar.offset[count]>1.e-7  ){
      fprintf(fplog,"|- OFFSET IS %f \n",colvar.offset[count]);}

...
// committors analysis
      ...
      for(i=0;i<commit.ncv;i++){
        ...
        if(sscanf(input.words[iline][0],"%lf",&uno)!=1) plumed_error("TO DEFINE A COMMITMENT REGION 4 NUMBERS ARE NEEDED!\n");
        if(sscanf(input.words[iline][1],"%lf",&due)!=1) plumed_error("TO DEFINE A COMMITMENT REGION 4 NUMBERS ARE NEEDED!\n");
        if(sscanf(input.words[iline][2],"%lf",&tre)!=1) plumed_error("TO DEFINE A COMMITMENT REGION 4 NUMBERS ARE NEEDED!\n");
        if(sscanf(input.words[iline][3],"%lf",&quattro)!=1) plumed_error("TO DEFINE A COMMITMENT REGION 4 NUMBERS ARE NEEDED!\n");
        ix=commit.index[i];
...
// multiple walkers (shared file)
      ...
      fprintf(mtd_data->fplog, "\n");
    } else if(!strcmp(word[0],"MM-META")){
      logical.metadymer=1;
      logical.dymerlocal=1;
      fprintf(mtd_data->fplog, " MINIMUM-MODE METADYNAMICS SIMULATION; REMARK: INVERSION NON ACTIVE(NOT COMPATIBLE) \n");
      int read_aver_mmeta = 0;
      real f_nt_medy, f_nt_hills, f_ratio_medy; 
      for(iw=1;iw<nw;iw++){
        if(!strcmp(word[iw],"AVER")){
          iw++; sscanf(word[iw], "%i", &(colvar.nt_medy)); read_aver_mmeta = 1;
          if(!read_aver_mmeta) plumed_error("MM-META SIMULATION ERROR: \"AVER\" KEYWORD NOT SPECIFIED");
          if(read_aver_mmeta==1){
            f_nt_medy=colvar.nt_medy;
            f_nt_hills=hills.nt_hills;
            f_ratio_medy=floor(f_nt_medy/f_nt_hills); 
            if(f_nt_medy/f_nt_hills!=f_ratio_medy) plumed_error("WITH MM-META \"AVER\" MUST BE A MULTIPLE OF \"W_STRIDE\""); 
          }
        } 
        if(!strcmp(word[iw],"NONLOCAL")){
          logical.dymerlocal=0;
        }
        if(logical.dymerlocal==1){
          if(!strcmp(word[iw],"NDIM")){
            iw++; sscanf(word[iw], "%i", &colvar.ndim_medy);
          } 
          if(!strcmp(word[iw],"DELTA_LOC1")){
            iw++; sscanf(word[iw], "%lf", &uno); colvar.delta_loc_medy1 = (real) uno;
          }          
          if(!strcmp(word[iw],"DELTA_LOC2")){
            iw++; sscanf(word[iw], "%lf", &uno); colvar.delta_loc_medy2 = (real) uno; 
          }
          if(colvar.delta_loc_medy2>colvar.delta_loc_medy1) plumed_error("DELTA_LOC2 MUST BE < THAN DELTA_LOC1");
          if(!strcmp(word[iw],"WFACTOR")){
            iw++; sscanf(word[iw], "%lf", &uno); colvar.wfact_loc_medy = (real) uno;
          }        
          if(colvar.wfact_loc_medy<0||colvar.wfact_loc_medy>1)  plumed_error("WFACTOR MUST BE BETWEEN 0 AND 1");
          if(!strcmp(word[iw],"WFACT_THR")){
            iw++; sscanf(word[iw], "%lf", &uno); colvar.force_thr_dy = (real) uno;
            logical.thr_dy=1;
          }
        } 
      }
      if(logical.dymerlocal==1){
        fprintf(mtd_data->fplog, "|-MINIMUM-MODE METADYNAMICS WITH AVER %i NDIM %i DELTA_LOC1 %f DELTA_LOC2 %f AND WFACTOR %f \n", colvar.nt_medy,colvar.ndim_medy,colvar.delta_loc_medy1,colvar.delta_loc_medy2,colvar.wfact_loc_medy);
        if(logical.thr_dy==1) {
          fprintf(mtd_data->fplog, "|-MINIMUM-MODE METADYNAMICS WITH THRESHOLD %f ON THE MAXIMUM FORCE ORTHOGONAL TO THE PRINCIPAL DIRECTION \n", colvar.force_thr_dy);
        }
//        fprintf(mtd_data->fplog, " %f %f \n", colvar.nt_medy/hills.nt_hills,floor(colvar.nt_medy/hills.nt_hills));
      }
      if(logical.dymerlocal==0){
        fprintf(mtd_data->fplog, "|NON-LOCAL-MINIMUM-MODE METADYNAMICS WITH AVER %i \n", colvar.nt_medy);
        fprintf(mtd_data->fplog, "|REMARK: ONLY 1 DIMENSION, \"NDIM\", \"DELTA_LOC1\", \"DELTA_LOC2\"AND \"WFACTOR\" WILL NOT BE READ \n");
      }
      ...
      count++;
    } else if(!strcmp(word[0],"PPII")) {
      read_ppii(word, count, &input,&iline,mtd_data->fplog);
      count++;
...
/* Atom position */      
      ...
      colvar.type_s[count]   = 60;
      read_camshift(word, count,&input, mtd_data->fplog);
      count++;
    } else if(!strcmp(word[0],"CSMETHYLS")) {
      colvar.type_s[count]   = 62;
      read_cs3meth(word, count,&input, mtd_data->fplog);
      count++;
    } else if(!strcmp(word[0],"NOE")) {
      colvar.type_s[count]   = 64;
      read_noe(word, count,&input, mtd_data->fplog);
      count++;
#endif
      ...
      count++;
    } else if(!strcmp(word[0],"PHIVALUES")) {
      colvar.type_s[count]   = 65;
      read_phivalues(word, count, &input, mtd_data->fplog);
      count++;
    } else if(!strcmp(word[0],"INTRACONTACT")){
      read_intracontact(word, count, &input, &iline, mtd_data->fplog);
      colvar.type_s[count]   = 66;
      count++;
...
// then we parse the line
  ...
  // #### -------------------------------------------------------------------------------
      ...
      for(iw=1;iw<nw;iw++){
        ...
        } else if(!strcmp(word[iw],"TARGETV")) {
          iw++; sscanf(word[iw], "%lf", &uno); cvsteer.targetV[icv-1]=(real) uno*mtd_data->eunit; 
          iw++; sscanf(word[iw], "%i", &tt); cvsteer.timeV[icv-1]=(int) tt; 
#if defined (PLUMED_GROMACS4)|| defined (PLUMED_GROMACS45)
        } else if(!strcmp(word[iw],"ANNEALING")) {
          iw++; sscanf(word[iw], "%lf", &uno); cvsteer.annealing[icv-1]=(real) uno*mtd_data->eunit; read_anneal=1;
        } else if(!strcmp(word[iw],"ANN_STRENGTH")) {
          iw++; sscanf(word[iw], "%lf", &uno); cvsteer.annstr[icv-1]=(real) uno;
#endif
        } else if(!strcmp(word[iw],"ADDTOENE")) {
          cvsteer.addtoene[icv-1]=1;
        } else if(!strcmp(word[iw],"RESTART")){
          logical.append = 1;
        } else {
      ...
      for(iw=1;iw<nw;iw++){ 
        ...
        } else if(!strcmp(word[iw],"DIFFUSION")) {
          iw++; sscanf(word[iw], "%lf", &uno); abmd.diff[icv-1]=(real) uno*mtd_data->eunit; read_diff=1;
        } else if(!strcmp(word[iw],"SEED")){
          iw++; sscanf(word[iw], "%i", &abmd.seed); 
        } else {
    ...
    } else if(!strcmp(word[0],"PROBRES")){


      logical.do_probres = 1;
      iw = seek_word(word,"CV");
      if(iw>=0){ sscanf(word[iw+1], "%i", &icv);
      } else{plumed_error("WITH PROBRES YOU ALWAYS HAVE TO SPECIFY THE \"CV\" KEYWORD\n");}
      fprintf(mtd_data->fplog, "APPLY PROBABILITY RESTRAINT ON CV %i:\n", icv); 
      iw = seek_word(word,"EQUIL_FRAMES");
      if (iw>=0) { sscanf(word[iw+1],"%i",&(hills.equil_frames_pres[icv-1]));
        fprintf(mtd_data->fplog, "PROBABILITY RESTRAINT: USING INITIAL EQUILIBRATION OVER %i FRAMES for CV %i \n", hills.equil_frames_pres[icv-1],icv);
      }
      iw = seek_word(word,"EQUIL");
      if (iw>=0) {
         logical.equil_probres[icv-1]=1;
         if(hills.equil_frames_pres[icv-1]>0) plumed_error("PROBABILITY RESTRAINT: EQUILIBRATION OVER FRAMES AND AUTOMATIC ARE NOT COMPATIBLE \n");
         fprintf(mtd_data->fplog, "PROBABILITY RESTRAINT: USING AUTOMATIC EQUILIBRATION SETUP for CV %i \n",icv);       
//hills.toll_pres
         iw = seek_word(word,"TOLERANCE"); 
         if (iw>=0) {
            sscanf(word[iw+1],"%lf",&uno);
            hills.toll_pres[icv-1]= (real) uno;  
            fprintf(mtd_data->fplog, "PROBABILITY RESTRAINT: TOLERANCE FOR AUTOMATIC EQUILIBRATION of CV %i is %f \n",icv,hills.toll_pres[icv-1]);
         }
      }
      iw = seek_word(word,"PROBFILE");
      if (iw>=0) {
        logical.probres[icv-1]=1;
        fprintf(mtd_data->fplog,"|--LOADING PROBABILITY RESTRAINT FILE for CV %i \n",icv);
        load_probresfile(word[iw+1],mtd_data->fplog,icv);
      } else {plumed_error("WITH PROBRES YOU ALWAYS HAVE TO SPECIFY A PROBABILITY FILE \n");}
      ...
      if (logical.invert[icv-1][0]==0) {
        if (logical.invert[icv-1][1]==0) plumed_error("NO LIMITS FOR INVERSION FOUND: SPECIFY AT LEAST ONE!");
      }
...
// in case initialize grid stuff
   for(i=0;i<grid.ncv;i++) if(logical.interval[grid.index[i]]) 
     plumed_error("GRID and INTERVAL are not yet compatible!!!!!!!!!!!\n");
...
// check if INTERVAL is used more than once
  ...
  if(tmpc>1 && logical.do_probres==0) plumed_error("INTERVAL CAN BE USED ONLY ON A SINGLE DIMENSION, ON MANY DIMENSIONS ITS BEHAVOUR IS NOT TESTED!\n");
...
void PREFIX read_defaults()
{
  ...
  logical.dymerlocal            = 1;
  logical.thr_dy                = 0;
  colvar.force_thr_dy           = 20;
  colvar.ndim_medy              = 1;
  colvar.wfact_loc_medy         = 0.5;
  colvar.delta_loc_medy1        = 10.0;
  colvar.delta_loc_medy2        = 1.0;
  colvar.nt_medy                = 10000;
  logical.metadymer             = 0;
  logical.do_probres            = 0;
  logical.tamd                  = 0;
  logical.debug                 = 0;
  logical.parallel_hills        = 0;
  logical.norescale             = 0;
  ...
  for(icv=0;icv<nconst_max;icv++){
    hills.toll_pres[icv]        = 0.3; // PROBRES 
    logical.abmd[icv]	 	= 0;
    logical.steer[icv]          = 0; 
    ...
    logical.probres[icv]        = 0;
    logical.equil_probres[icv]  = 0; // PROBRES
    hills.equil_frames_pres[icv]= 0; // PROBRES
    ...
    colvar.cvatoms[icv]         = NULL;
    colvar.averaging[icv]       = 0;
    grid.min[icv]               = 0.;
    grid.max[icv]               = 0.; 
    grid.lbox[icv]              = 0.;
    grid.dx[icv]                = 0.;
    ...
    cvsteer.annstr[icv]      = 1.0;
    cvsteer.addtoene[icv]       = 0;
    cvsteer.targetV[icv]          = 0.;
    cvsteer.timeV[icv]            = 0;
    cvsteer.accV[icv]             = 0.;
    cvsteer.naccV[icv]            = 0 ;
...
int PREFIX  load_probresfile  (char *filename, FILE *fplog, int icv){
    FILE *fp;
    char *str, stringa[200];
    int  ii;
    double dummy;

    fprintf(fplog,"|--LOADING PROBRES FILE\n");
    fp=fopen(filename,"r");
    if (fp == NULL) {
       char buf[1024];
    ...
    while(1){
       ...
       sscanf(str, "%lf", &dummy);
       hills.probres_cv[hills.probrespoints[icv-1]][icv-1] = (real) dummy;
       str =(char *) strtok(NULL, " \t");
       sscanf(str, "%lf", &dummy);
       hills.probres_val[hills.probrespoints[icv-1]][icv-1]= (real) dummy;
       str =(char *) strtok(NULL, " \t");
       hills.probrespoints[icv-1]++;
       if(hills.probrespoints[icv-1]>MAXPROBRES-1) {
         char buf[1024];
         sprintf(buf,"MAXIMUM NUMBER OF POINTS FOR PROBRES FILE IS %i",MAXPROBRES);
         sprintf(buf,"INCREASE MAXPROBRES VARIABLE");
         plumed_error(buf);
       }
    }
    return 1;

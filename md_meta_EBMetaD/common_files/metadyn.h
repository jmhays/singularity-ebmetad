...
#define __METADYN_H
   ...
   #define MYEXT
#elif defined PLUMED_GENERIC_CXX
   #define MYEXT
#else
   #define MYEXT extern
#endif   
...
//Generic CXX
#elif defined (PLUMED_GENERIC_CXX)
#define PREFIX PlumedGenericCxx::
#include <set>
...
// verlet list
#define MAXNN 2000
#define MAXPROBRES 10000                                // mesh of probability restraint file 
...
struct rmsd_container_t {
	...
	int saverunning;
	int saverunningfreq;
	char saverunningfile[200];
	FILE *fpsaverunningfile;
};
...
// previously static variables:
   FILE*  hills_file;
   int    hills_push_first;
   int    hills_add_first;
   int    last_hill_at_this_step;
   FILE*  debug_grid_file;
   FILE*  cv_file;
   int    sum_nalloc;
   real*  sum_buf;
   int    sumi_nalloc;
   ...
   real*  intersum_buf;
   rvec*  puckering_R;
   int    testderivatives_first;
   rvec** testderivatives_pos;
   rvec** testderivatives_der;
   rvec** testderivatives_previous_pos;
   rvec** testderivatives_previous_der;
   real*  testderivatives_integral;
   FILE*  testderivatives_debug_file;
#ifdef __PLUMED_MPI
...
#elif defined (ACEMD) || defined (AMBER) || defined (DRIVER) || defined (STANDALONE) || defined (PLUMED_QESPRESSO) || defined (PLUMED_CP2K)
   int  imcon;
   real cell[3];
...
struct logical_s
{
  ...
  int    norescale;
  int    welltemp;
  int    do_probres;                                    // Activate probability restraint
  int    probres[nconst_max];                           // Apply probability restraint on a specific CV
  int    equil_probres[nconst_max];                     // Use equilibration period on probability restraint
  int    metadymer;                                     // Activate MM-META
  int    dymerlocal;                                    // LOCAL MM-META   
  int    thr_dy;                                        // Activate threshold for MM-META force  
...
struct colvar_s
{
  ...
  real   offset  [nconst_max];                          // Used by torsion so to add an offset 
  ...
  int    nconst_active;                                 // number of active VARS usefull for MM-META
  int    iactive[nconst_max];                           // active VARS usefull for MM-META 
  int    nt_medy;                                       // delta_t (AVER)->MM-META
  real   delta_loc_medy1;                               // DELTA 1->MM-META
  real   delta_loc_medy2;                               // DELTA 2->MM-META
  int    ndim_medy;                                     // NDIM->MM-META
  real   wfact_loc_medy;                                // WFACT->MM-META
  real   force_thr_dy;                                  // threshold on the force MM-META 
  real   max_force_dy;                                  // maximum force MM-META
  real   wfact_loc_medy_tmp;                            // temporary WFACT->MM-META used when threshold on the force is active  
  int    num_medy;                                      // nstep counter->MM-META
  int    num_medy2;                                     // nstep counter for second interval->MM-META
  int    count_medy;                                    // count the number of intervals (AVER) in which the direction is averaged; after NDIM intervals the directions are computed->MM-META
  int    *count_dim_medy;                               // This is to save the directions at every step->MM-META
  int **vercomp;                                        // versor component index and depends on NDIM->MM-META
  real **medy;                                          //  previous interval colvar, to be used in the calculation of the displacement vector->MM-META 
  real *medy2;                                          // sum of the difference vector components->MM-META
  real *medy_ver;                                       // versor of the principal direction->MM-META
  real ***medy_ver_loc;                                 // versors component, LOCAL-->MM-META
...
struct hills_s
{
  ...
  real     *ww_wfact;                                   // wfact for each hills->MM-META
  real     **medy_ver_nh;                               // versor component for each HILLS, NON LOCAL->MM-META
  real     ***medy_ver_loc_nh;                          // versors components for each HILLS, LOCAL->MM-META
  int      max_stride;    				// Maximum stride between hills (0 means NO maximum)
  real     *ww;						// Hill height history
  real     **ww_pres;                                   // Hill height history in probability restraint 
  ...
  int      probrespoints[nconst_max];                   // number of points of probability restraint function 
  real     cv_tot_pres[nconst_max];                     // cumulative CV usefull for probability restraint
  real     cv_tot_pres_half[nconst_max];                // cumulative CV at half time usefull for probability restraint
  real     cvsq_tot_pres[nconst_max];                   // cumulative square CV usefull for probability restraint
  real     cvsq_tot_pres_half[nconst_max];              // cumulative square CV at half time usefull for probability restraint
  real     av_cv_pres[nconst_max];                      // reference average CV usefull for probability restraint
  real     std_cv_pres[nconst_max];                     // reference CV standard deviation usefull for probability restraint
  real     av_cv_pres_min[nconst_max];                  // stored average CV usefull for probability restraint
  real     std_cv_pres_min[nconst_max];                 // stored  CV standard deviation usefull for probability restraint
  int      count_tot_pres[nconst_max];                  // number of frames used for average hills height in probability restraint  
  int      count_pres[nconst_max];                      // number of frames used for time equilibration in probability restraint 
//  int      count_tot_pres_half[nconst_max];             // number of frames used for hills height at half time in probability restraint
  int      count_tot_pres_hills[nconst_max];            // number of hills counts used for hills height at half time in probability restraint 
  real     probres_cv[MAXPROBRES][nconst_max];          // probability restraint function CV
  real     probres_val[MAXPROBRES][nconst_max];         // probability restraint function VALUE
  real     step_pres[nconst_max];                       // grid interval of probres function
  real     toll_pres[nconst_max];                       // automatic equilibration tollerance = hills.toll_pres*sigma_ref
  int      equil_frames_pres[nconst_max];               // equilibration frames for probability restraint
...
struct steer {
 ...
 real    annstr[nconst_max];
 int     addtoene[nconst_max];
 int     sign    [nconst_max];
 ...
 real   targetV [nconst_max];
 int   timeV [nconst_max];
 real   accV    [nconst_max];
 int    naccV   [nconst_max];
} ;
...
#if defined (AMBER) || defined (STANDALONE) || defined (PLUMED_CP2K)
 real   *oldforce; 
 real   *oldvel; 
#endif
} ;
...
struct abmd {
 ...
 real    diff    [nconst_max];                          // start position
 int     seed;
} ;
...
// contains Pietropaolo index of atoms and values 
struct chirality_frameset { 
     int **atomlist; //meant to be as general as possible to include longer lists 
     real *vals;	
};
...
struct sz_data {
        ...
        struct chirality_frameset **chiral_frameset;
...
class Plumed
{
 ...
 void sfree(void *ptr);
#elif defined(PLUMED_GENERIC_CXX)
class PlumedGenericCxx
{
bool collect_garbage;
std::set<void*>    garbage;
std::set<unsigned> unique;
void* calloc(size_t num,size_t size){void*p=::calloc(num,size); if(p) garbage.insert(p); return p; }
void* malloc(size_t size){void*p=::malloc(size); if(p) garbage.insert(p); return p; }
void* realloc(void* ptr,size_t size){void*p=::realloc(ptr,size); if(ptr) garbage.erase(ptr); if(p) garbage.insert(p); return p;}
void  free(void* ptr){::free(ptr); if(ptr) garbage.erase(ptr); }

public:
 PlumedGenericCxx(){
   collect_garbage=true;
   mtd_data.hills_file=NULL;
   mtd_data.debug_grid_file=NULL;
   mtd_data.cv_file=NULL;
   mtd_data.testderivatives_debug_file=NULL;
   mtd_data.fplog=NULL;
 }
 void disableGarbageCollector(){
   collect_garbage=false;
 }
 const std::set<unsigned> & getAtoms(){
   unique.clear();
   for(int i_c=0;i_c<colvar.nconst;i_c++)
     for(int i=0;i<colvar.natoms[i_c];i++)
       unique.insert(colvar.cvatoms[i_c][i]);
   return unique;
 }
 virtual void pbc(const double*p1,const double*p2,double*d)const{
   d[0]=p1[0]-p2[0];
   d[1]=p1[1]-p2[1];
   d[2]=p1[2]-p2[2];
 }
 virtual ~PlumedGenericCxx(){
   if(mtd_data.hills_file) fclose(mtd_data.hills_file);
   if(mtd_data.debug_grid_file) fclose(mtd_data.debug_grid_file);
   if(mtd_data.cv_file) fclose(mtd_data.cv_file);
   if(mtd_data.testderivatives_debug_file) fclose(mtd_data.testderivatives_debug_file);
   if(mtd_data.fplog) fclose(mtd_data.fplog);
   if(collect_garbage) for(std::set<void*>::iterator p=garbage.begin();p!=garbage.end();++p) ::free(*p);
 }

#ifdef __PLUMED_MPI
 void set_comm(MPI_Comm comm);
#endif
 void init_metadyn(int atoms,real ddt,real *mass,real *charge,real myboltz,real factor_angstrom,char *metain);
 void meta_force_calculation(int istep, real*xxx,real*fff,real *energy);
private:
 void mtd_data_init(int atoms,real ddt,real *mass,real *charge,real myboltz,char *metain);
...
#elif defined(DL_POLY) || defined(AMBER) || defined(PLUMED_QESPRESSO) || defined (GAT_LJONES) || defined (PLUMED_CP2K)
...
// HILLS stuff
 ...
 real hills_engine_probres(real* ss0, int comp);
 void hills_force();
 ...
 void mm_meta_direction(); // calculate the direction for MM-META
 void mm_meta_init(); // inititialize and allocate VARS for MM-META
 real mm_meta_force(int ih,real* ss0,real* dp, real* force); // calculate bias potential and force for MM-META
 void mm_meta_hills_assign(struct mtd_data_s *mtd_data,int numhills); // add new hills for MM-META vectors
 void mm_meta_hills_write(struct mtd_data_s *mtd_data,int numhills); // write new hills for MM-META vectors 
 void mm_meta_ortho_hills(long int line);
 void mm_meta_read_hills(struct  mtd_data_s *mtd_data,long int line,int j, int nact,int* act_to_ind, char* str);  
 void mm_meta_max_force(); // calculate maximum force in the direction orthogonal to the principals for MM-META
...
// reading...
 ...
 int  read_phivalues     (char **word,int count,t_plumed_input *input,           FILE *fplog);
 int  read_intracontact  (char **word,int count,t_plumed_input *input,int *iline,FILE *fplog);
...
// these variables read lines directly from the input, thus they need the iline pointer to increment it
 ...
 int  read_ppii          (char **word,int count,t_plumed_input *input,int *iline,FILE *fplog);
 ...
 int  load_probresfile   (char *word, FILE *fplog, int icv);
...
// calculating
 ...
 void ppii_restraint          (int i_c, struct mtd_data_s *mtd_data);
 ...
 void phivalues_restraint     (int i_c, struct mtd_data_s *mtd_data);
 void intracontact_restraint  (int i_c, struct mtd_data_s *mtd_data);
 ...
 void reparam_bernd_path      ( struct sz_data *pmy_sz , real maxerr, int maxiter , real smooth);
 void interpolate_bernd_path   ( struct sz_data *pmy_sz , int npts );
 ...
 void cs3meth_restraint       (int i_c, struct mtd_data_s *mtd_data);
 void cs3methens_restraint       (int i_c, struct mtd_data_s *mtd_data);
 int  read_cs3meth      (char **word,int count,t_plumed_input *input,           FILE *fplog);
 int  read_noe      (char **word,int count,t_plumed_input *input,           FILE *fplog);
 void noe_restraint      (int i_c, struct mtd_data_s *mtd_data);
 #if defined (PLUMED_GROMACS) && defined (__cplusplus)
 } 
 #endif
...
// misc routines
 ...
 int  read_sz_chiral(struct sz_data *my_sz, FILE *fplog);
...
void msd_elem_diff(struct hybrid_elem *diff,struct hybrid_elem  *e1,struct hybrid_elem  *e2,struct hybrid_elem  *e3, FILE *fplog, int dorandom, int print_rot_mat);
...
void msd_calculation_dynamic(struct rmsd_workstruct_s *work,int der_frameref_on);
...
void reparam_with_multiple_matrix( struct sz_data *pmy_sz , char *filename, real maxerr, int maxiter, real smooth);
void interpolate_with_multiple_matrix( struct sz_data *pmy_sz , char *filename, int npts);
...
int get_matrix_size(struct sz_data *my_sz);
void average_matrix(int i, int j,struct sz_data *my_sz);	
...
// real hbd_metrics_target ( struct hybrid_elem *run,  struct hybrid_elem *ref );
 ...
 int  hbd_collect_jacobian ( struct hybrid_frameset *running, real ** mathybrid ,  real ** myinverse, FILE *fplog , int absval, int diag );
...
#if defined(DL_POLY) || defined(AMBER) || defined(PLUMED_QESPRESSO) || defined(PLUMED_CP2K)  
#define min(a, b) (((a) < (b)) ? (a) : (b))
#endif

#if defined (NAMD) || defined (LAMMPS_PLUMED) || defined (PLUMED_CPMD) || defined (PLUMED_GENERIC_CXX)

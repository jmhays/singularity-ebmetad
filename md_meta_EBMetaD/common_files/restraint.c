...
{
  int i_c, ncv, nth, ntrh, ntp, rpxm, ntwg, mmth;                    // indexes for cycles and time
...
// eventually dump atoms
  ...
  for(i_c=0;i_c<ncv;i_c++){
    ...
    dafed[i_c].f         = 0.;                  // initialization of dafed forces
...
//    if((!logical.always[i_c])&&(logical.rpxm)&&(!rpxm)&&(!ntp)) continue;
    ...
    switch(colvar.type_s[i_c]){
      ...
      case 17: ppii_restraint(i_c, mtd_data); break;                 	// DIHEDRAL-COR
      // water CVs
...
#ifdef HAVE_ALMOST
      case 60: camshift_restraint(i_c, mtd_data); break;                // CAMSHIFT ENERGY
      case 61: camshiftens_restraint(i_c, mtd_data); break;             // ENS CAMSHIFT ENERGY
      case 62: cs3meth_restraint(i_c, mtd_data); break;                 // CAMSHIFT ENERGY
      case 63: cs3methens_restraint(i_c, mtd_data); break;              // CAMSHIFT ENERGY
      case 64: noe_restraint(i_c, mtd_data); break;             	// CAMSHIFT ENERGY
#endif
      case 65: phivalues_restraint(i_c, mtd_data); break;                    // CMAP 
      case 66: intracontact_restraint(i_c, mtd_data); break;	              	// COORD
    }
  ...
  if(logical.metadymer==1 && logical.not_same_step && !firstTime ) { // calc MM-META direction
    mm_meta_direction();
  }
  ...
  if(logical.do_hills){
    ...
    if(logical.metadymer==1 && logical.dymerlocal==1 && colvar.count_medy>colvar.ndim_medy && logical.thr_dy==1) { // MM-META using threshold on the maximum force
      mm_meta_max_force(); // 
    }
    if(logical.widthadapt) hills_adapt();			// to adapt gaussian width
    if(nth && logical.metadymer==0) {
      hills_add(mtd_data);                                      // add HILL
    } 
    if(nth && logical.metadymer==1 && colvar.count_medy> 1) {                           // add HILL MM-META
      if(logical.dymerlocal==0) {
        hills_add(mtd_data); 
      }
      if(logical.dymerlocal==1 && colvar.count_medy>colvar.ndim_medy) { // add HILL MM-META local
        if(logical.thr_dy==0) {
          hills_add(mtd_data);
        }
        if(logical.thr_dy==1) { // add HILL MM-META using threshold on the maximum force 
          if(colvar.max_force_dy>=colvar.force_thr_dy) {
            colvar.wfact_loc_medy=0;
          }
          if(colvar.max_force_dy<colvar.force_thr_dy) {
            colvar.wfact_loc_medy=colvar.wfact_loc_medy_tmp*(1-(colvar.max_force_dy/colvar.force_thr_dy));
          }
          colvar.max_force_dy=0;
          hills_add(mtd_data);
        }
      } 
    }
...
{ 
  ...
  if(logical.energy) for(i_c=0;i_c<colvar.nconst;i_c++) if(colvar.type_s[i_c]==35) icv_ene = i_c;
  ...
  for(i_c=0;i_c<colvar.nconst;i_c++) if(logical.steer[i_c]){
    ...
    if(firstTime){
      ...
      if (cvsteer.targetV[i_c]>0.){
        fprintf(mtd_data.fplog,"|- ACCUMULATING POTENTIAL FOR TUNING VAR %d SO K TO HAVE %f ENE IN %d TIME \n",i_c+1,cvsteer.targetV[i_c],cvsteer.timeV[i_c]);
        fflush(mtd_data.fplog);
      }
    ...
    if(cvsteer.annealing[i_c]>0.) { 
      ff = - pow((cvsteer.annealing[i_c]/mtd_data.temp_t),cvsteer.annstr[i_c])*cvsteer.spring[i_c]*tmp
           - pow((cvsteer.annealing[i_c]/mtd_data.temp_t),cvsteer.annstr[i_c])*cvsteer.slope[i_c];
      myV = 0.5*pow((cvsteer.annealing[i_c]/mtd_data.temp_t),cvsteer.annstr[i_c])*cvsteer.spring[i_c]*tmp*tmp 
           +cvsteer.slope[i_c]*pow((cvsteer.annealing[i_c]/mtd_data.temp_t),cvsteer.annstr[i_c])*tmp;
      V  += myV;
    } else {
      ff = -cvsteer.spring[i_c]*tmp - cvsteer.slope[i_c];
      myV = 0.5*cvsteer.spring[i_c]*tmp*tmp + cvsteer.slope[i_c]*tmp;
      V  += myV;
    }
    if(force) force[i_c] += ff;
    if(cvsteer.addtoene[i_c]&&logical.energy) colvar.ss0[icv_ene]+=myV;
    if ( cvsteer.targetV[i_c]>0){
        cvsteer.accV[i_c]+=myV;
        cvsteer.naccV[i_c]++;
        if(cvsteer.naccV[i_c]>cvsteer.timeV[i_c]){
             cvsteer.spring[i_c]=cvsteer.spring[i_c]*cvsteer.targetV[i_c]/(cvsteer.accV[i_c]/cvsteer.naccV[i_c]);  
             fprintf(mtd_data.fplog,"|- K IS NOW EVALUATED TO BE %f FOR CV %d \n",cvsteer.spring[i_c],i_c+1);  
             cvsteer.targetV[i_c]=0.;
        }
    }
...
real PREFIX abmd_engine(real* ss0, real* force)
{
  ...
  for(i_c=0;i_c<colvar.nconst;i_c++)if(logical.abmd[i_c]){
    ...
    if(abmd.now[i_c]<=abmd.diff[i_c]) abmd.diff[i_c]=0.;
    noise = (2.0*rando(&abmd.seed)-1.0)*abmd.diff[i_c];
...
//  if( reconOn==1 && reconinpt.monitor==1 ){ fprintf(cv_file, " current_basin"); }
  ...
  for(i_c=0;i_c<colvar.nconst;i_c++){  
              if(logical.steer[i_c] ){
                  if(cvsteer.timeV[i_c]==0){ 
                   fprintf(cv_file," XX XX RST%d WORK%d",i_c+1,i_c+1);
                  }else {
                   fprintf(cv_file," XX XX RST%d KAPPA%d WORK%d",i_c+1,i_c+1,i_c+1);
                  }
              } 
  }
...
//#endif
    ...
    if(!mtd_data.cv_file) mtd_data.cv_file = fopen((mtd_data.ionode?mtd_data.colfilen:"/dev/null"), "a");
/*
...
*/
    fprintf(mtd_data.cv_file, "%10.4f", time_s);
    for(i_c=0;i_c<colvar.nconst;i_c++) fprintf(mtd_data.cv_file, "   %14.9f", colvar.ss0[i_c]);
#ifdef CVS
...
//       for(i=0;i<colvar.bespoke_ncv;i++){ cv_in[i]=colvar.ss0[ colvar.bespoke_cvlist[i] ]; }
       ...
       fprintf(mtd_data.cv_file, "   %14.9f", bespoke_err); 
    }
#endif
    if (logical.do_hills) fprintf(mtd_data.cv_file, " %14.9f ",hills.Vhills/mtd_data.eunit);
    if (logical.do_walls) fprintf(mtd_data.cv_file, " %14.9f ",cvw.Vwall/mtd_data.eunit);
    if (logical.do_constraint) fprintf(mtd_data.cv_file, "  %14.9f ",Vconstr/mtd_data.eunit);
    if (logical.do_steerplan) fprintf(mtd_data.cv_file, "  %14.9f ",Vsteerplan/mtd_data.eunit);
    if (logical.do_external) fprintf(mtd_data.cv_file, "  %14.9f ",Vext/mtd_data.eunit);
#ifdef RECONMETAD
#ifndef DRIVER
    if( reconOn==1 ){ fprintf(mtd_data.cv_file, "   %14.9f", Vrecon/mtd_data.eunit ); }
//    if( reconOn==1 && reconinpt.monitor!=1 ){ fprintf(mtd_data.cv_file, "   %14.9f", Vrecon/mtd_data.eunit ); }
//    if( reconOn==1 && reconinpt.monitor==1 ){ fprintf(mtd_data.cv_file, "   %14d", current_basin ); }
#endif
#endif
    for(i_c=0;i_c<colvar.nconst;i_c++){  if(logical.steer[i_c]){
             if(cvsteer.timeV[i_c]==0){
               fprintf(mtd_data.cv_file," RST %d %14.9f %20.9lf",i_c+1,cvsteer.pos[i_c], cvsteer.work[i_c] );} 
             else {
               fprintf(mtd_data.cv_file," RST %d %14.9f %14.9f %20.9lf",i_c+1,cvsteer.pos[i_c],cvsteer.spring[i_c],cvsteer.work[i_c] );} 
             }
    }
    for(i_c=0;i_c<colvar.nconst;i_c++){  if(logical.abmd[i_c]){fprintf(mtd_data.cv_file," ABMD %d %14.9f ",i_c+1, abmd.min[i_c] );} }
    for(i_c=0;i_c<colvar.nconst;i_c++){  if(logical.cnstr[i_c]){fprintf(mtd_data.cv_file," CONSTR %d %14.9f %20.9lf ",i_c+1,cvcnstr.pos[i_c],cvcnstr.energy[i_c]);} }
    if(logical.do_steerplan)fprintf(mtd_data.cv_file,"%s",steerplan.log)  ;
    if(colvar.pg.nlist!=0)  fprintf(mtd_data.cv_file, "%s",colvar.pg.log ); 
    for(i_c=0;i_c<colvar.nconst;i_c++){ if(logical.dafed[i_c]){ print_dafed(&dafed[i_c], mtd_data.cv_file, i_c); } }
    for(i_c=0;i_c<colvar.nconst;i_c++){  if(logical.tamd && colvar.on[i_c]){fprintf(mtd_data.cv_file," %14.9f ",tamd.pos[i_c]);} }
    if(logical.tamd) {fprintf(mtd_data.cv_file," %14.9f ",tamd.drift);};
    fprintf(mtd_data.cv_file, "\n");
    fflush(mtd_data.cv_file);
#ifdef STANDALONE 
    fclose(mtd_data.cv_file);
#endif
}
...
//    if( reconOn==1 && reconinpt.monitor!=1 ) ddr+= recon_der[i_c];      // Reconnaissance metadynamics forces
    ...
    for(i=0;i<colvar.natoms[i_c];i++) {
      ...
      if(cvsteer.addtoene[i_c]&&logical.energy) {
        mtd_data->cforce[iat][0] += cvw.fwall[i_c]*colvar.myder[i_c][i][0]; 
        mtd_data->cforce[iat][1] += cvw.fwall[i_c]*colvar.myder[i_c][i][1]; 
        mtd_data->cforce[iat][2] += cvw.fwall[i_c]*colvar.myder[i_c][i][2];
      } 
#endif    
    }
  }
...
{ 
   ...
   //fprintf(mtd_data.fplog,"|- STEERPLAN TIME %f NEXTSTAGETIME %f\n",time,steerplan.nextstage_time);
   if(time>=steerplan.nextstage_time||firstTime){
       for(i=0;i<steerplan.totstages;i++){
       //    fprintf(mtd_data.fplog,"|- STEERPLAN  ACTION %d AT TIME %f\n",i,steerplan.actions[i].t);
...
//---------------------------------------------------------------------------------------------
void PREFIX mm_meta_max_force(){
  int ii, jj, icv;
  real force_dy_pr, force_dy;
  ...
  for(jj=0;jj<colvar.nconst_active;jj++) {
     if(jj>colvar.ndim_medy-1) {
       ...
       for(ii=0;ii<colvar.nconst_active;++ii){
          ...
          force_dy_pr+=colvar.medy_ver_loc[colvar.count_dim_medy[colvar.ndim_medy-1]-1][jj][ii]*colvar.ff_hills[icv]*colvar.delta_r[icv];
       }
       if(fabs(force_dy_pr)>force_dy) {
         force_dy=fabs(force_dy_pr);
       }
     }
     if(force_dy>colvar.max_force_dy) {
       colvar.max_force_dy=force_dy;
     }
  }
}

//------------------------------MINIMUM MODE METADYNAMICS-CALC DIRECTION-----------------------
void PREFIX mm_meta_direction(){
  int ndym,i,j,ii,jj,k,ortho, mmth;
  real norm_medy,norm_ver,diffpos,maxver,prodscal_medy[nconst_max]; 

  mmth  = ( (logical.not_same_step)&&(colvar.it%colvar.nt_medy==0) && colvar.it>0 );
  // frequency to calc MM-META direction

//  fprintf(stderr, "MM-META INIT PIPPO %i \n",colvar.iactive[colvar.nconst_active]); 
  norm_medy=0.;
  norm_ver=0;
  // start
  colvar.num_medy++;
  if(colvar.count_medy>=1) {
    colvar.num_medy2++;
    for(ii=0;ii<colvar.nconst_active;++ii) {
       i=colvar.iactive[ii];
       diffpos=colvar.ss0[i]-colvar.medy[ii][colvar.num_medy2-1]; 
       // difference vector component between the actual step and a previous step (AVER) 
       if(colvar.type_s[i]==5 || (  colvar.type_s[i]==34 &&  colvar.type[i]==2 )) {  
         // inserted periodicity  
         if(diffpos>M_PI) diffpos-=2.*M_PI;
         else if(diffpos<-M_PI) diffpos+=2.*M_PI;
       } // inserted periodicity (end)          
       colvar.medy2[ii]+=diffpos/colvar.delta_r[i]; 
       // sum the difference vector components divided for sigma, NOTE that this 
       // is the scaling factor of the MM-META simulation assuming that the metric 
       // matrix is diagonal, non diagonal metric matrix is not implemented yet
    }
  }
  for(ii=0;ii<colvar.nconst_active;++ii) {
     i=colvar.iactive[ii];
     colvar.medy[ii][colvar.num_medy-1]=colvar.ss0[i]; 
     // save CVs at the actual step (to be used then as previous step)
  }

  if(mmth) { // do every AVER steps
//    for(ii=0;ii<colvar.nconst_active;++ii) {
//       fprintf(stderr, "MM-META INIT PIPPO %i %f %f %i \n",colvar.it,colvar.medy2[ii]/colvar.num_medy,colvar.medy2[ii],colvar.num_medy);
//    }

//    fprintf(stderr, "MM-META INIT PIPPO %i \n",colvar.it);
    if(logical.dymerlocal==1) {
      if(colvar.count_medy<=colvar.ndim_medy) {
         colvar.count_medy++; // count the number of intervals (AVER) in which the 
                              // direction is averaged; after NDIM intervals the directions 
                              // are computed
      }
    }
    if(logical.dymerlocal==0) { // if metadymer active in non local version NDIM is 1 for default
      if(colvar.count_medy<=1) {
         colvar.count_medy++;
      }
    }
    if(colvar.count_medy==1) { // in the first interval initialize 
      for(ii=0;ii<colvar.nconst_active;++ii) {
         colvar.medy_ver[ii]=0;
         if(logical.dymerlocal==1) {
           for(jj=0;jj<colvar.nconst_active;++jj) {
              for(j=0;j<colvar.ndim_medy;++j) colvar.medy_ver_loc[j][ii][jj]=0;
           }
         }
      }
      colvar.num_medy=0;
    ...
    if(colvar.count_medy>1) { // after the first interval the principal direction can be calculated  
      norm_medy=0.;
      for(ii=0;ii<colvar.nconst_active;++ii) {
         colvar.medy_ver[ii]=colvar.medy2[ii]/colvar.num_medy; // averaged principal direction
         norm_medy+=(colvar.medy_ver[ii])*(colvar.medy_ver[ii]);
      }
      for(ii=0;ii<colvar.nconst_active;++ii) {
         colvar.medy_ver[ii]=(colvar.medy_ver[ii])/(sqrt(norm_medy)); 
         // versor of the principal direction
         // find the largest ver
         // this is to choose the largest component of the minimal curvature direction
         colvar.medy2[ii]=0.;
      }
      colvar.num_medy=0;
      colvar.num_medy2=0;
      if(logical.dymerlocal==1) {
        for(ndym=1;ndym<=colvar.ndim_medy;++ndym) {
           if(colvar.count_medy>ndym) { // after the first interval only the first direction 
                                        // can be computed after the second intervall the second 
                                        // and so on...; i.e, NDIM+1 intervals are needed to 
                                        // calculated the first NDIM principal directions 
             if(colvar.count_dim_medy[ndym-1]==colvar.ndim_medy) colvar.count_dim_medy[ndym-1]=0; 
                                        // This is to save the directions at every step
             colvar.count_dim_medy[ndym-1]++; // see below for explanations
             if(ndym==1) {
               for(ii=0;ii<colvar.nconst_active;++ii)  {
                  colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][ndym-1][ii]=colvar.medy_ver[ii];  
                                        // save the versor of the first direction
               }
             }
             if(ndym>1) { // This is to calculate the principal directions in the case more than 1 direction is required
                          // I take the example of three directions required. This will need four intervals (AVER)
                          // In in second interval the first direction is calculated, in the third interval the versor of the actual principal
                          // direction is projected in the orthogonal space of the first direction calculated in the first interval
                          // in this way the second direction is calculated, the actual first direction is saved and will be used after
                          // in the fourth intervall the third direction is calculated projecting the actual first direction in the space orthogonal 
                          // to the the second calculated in the third intervall and the first calculated in the second intervall.
                          // The actual first direction is saved and also the second direction orthogonal to the first direction saved
                          // in the third interval is caculated....the process is continued iteratively.  
               norm_ver=0;
               for(ii=0;ii<colvar.nconst_active;++ii) {
                  prodscal_medy[ii]=0;
                  for(i=ndym-1;i<colvar.nconst_active;++i) {
                     for(jj=0;jj<colvar.nconst_active;++jj) {
                        prodscal_medy[ii]+=colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][i][jj]*colvar.medy_ver[jj]*colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][i][ii]; 
                     // here the principal direction (actually calculated) is projected in the space 
                     // orthogonal to the first ndym-1 directions 

                     }
                  }
                  norm_ver+=prodscal_medy[ii]*prodscal_medy[ii];
               }
             }
             ...
             for(ii=0;ii<colvar.nconst_active;++ii) {
                if(ndym>1) {
                  colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][ndym-1][ii]=prodscal_medy[ii]/sqrt(norm_ver);
                }
                // orthogonalization gram-shmidt: once the NDIM directions are calculated the 
                // space is orthogonalized and normalized 

                if(ii==0) {
                  maxver=fabs(colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][ndym-1][ii]);
                  colvar.vercomp[colvar.count_dim_medy[ndym-1]-1][ndym-1]=ii;
                }
                if(ii>0) {
                  if(fabs(colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][ndym-1][ii])>=maxver) {
                    maxver=fabs(colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][ndym-1][ii]);
                    colvar.vercomp[colvar.count_dim_medy[ndym-1]-1][ndym-1]=ii;
                  }
                }
             ...
             for(ii=0;ii<colvar.nconst_active;++ii) {
                ...
                ortho=0;
                for(k=0;k<ndym;++k) {
                   if(ii==colvar.vercomp[colvar.count_dim_medy[ndym-1]-1][k]) {
                     ortho=1; // removing vector having largest component on the minimal 
                              // curvature direction
                   }
                }
                if(ortho==1) continue;
                ...
                norm_ver=0;
                for(jj=0;jj<colvar.nconst_active;++jj) {
                   if(jj!=ii) colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][j][jj]=0;
                   if(jj==ii) colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][j][jj]=1;
                   for(i=0;i<=j-1;++i){
                      colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][j][jj]=colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][j][jj]-colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][i][ii]*colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][i][jj];
                   }
                   norm_ver+=colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][j][jj]*colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][j][jj];
                }
                for(jj=0;jj<colvar.nconst_active;++jj) {
                   colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][j][jj]=colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][j][jj]/sqrt(norm_ver);
                }
             }
// CHECK ORTHO
//     for(ii=0;ii<colvar.nconst_active;++ii) {
...
//              norm_ver+=colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][ii][j]*colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][jj][j];
//           }
//           fprintf(stderr,"%i %i %i %f %f \n",colvar.it,ii,jj,norm_ver,colvar.medy_ver_loc[colvar.count_dim_medy[ndym-1]-1][ii][jj]);
//        }
//     }
// CHECK ORTHO             
           }
...
//------------------------------MINIMUM MODE METADYNAMICS-CALC DIRECTION END-------------------- 

                    ...
                    int nrepl, real rte0, real rteio, const t_commrec *mcr, FILE *fplog)
{
   ...
   mtd_data.cforce = float_2d_array_alloc(mtd_data.natoms,3);
   mtd_data.vel = NULL;
...
// clean local forces
   for(iat_dd=start;iat_dd<start+homenr;iat_dd++){
     ...
     mtd_data.cforce[iat][0]=0.0;
     mtd_data.cforce[iat][1]=0.0;
     mtd_data.cforce[iat][2]=0.0;
   }
...
// each process only updates the local atoms
   for(iat_dd=start;iat_dd<start+homenr;iat_dd++){
     iat=plumed_ll_index(iat_dd);
     if(logical.energy) {
       mtd_data.force[iat][0]+=-colvar.d_0[icv_ene]*(force[iat_dd][0]+mtd_data.cforce[iat][0]);
       mtd_data.force[iat][1]+=-colvar.d_0[icv_ene]*(force[iat_dd][1]+mtd_data.cforce[iat][1]);
       mtd_data.force[iat][2]+=-colvar.d_0[icv_ene]*(force[iat_dd][2]+mtd_data.cforce[iat][2]);
     }
...
void mtd_data_init(int atoms, real dt ,real *mass, real *charge, int *imcon, real *eunit, char *metainp)
{
 ...
 mtd_data.charge=(real *)calloc(atoms,sizeof(real));
   ...
   mtd_data.boltz=0.001987191; // kcal/mol/K
#elif AMBER 
   for (i=0;i<atoms;i++){mtd_data.charge[i]= charge[i]/18.2223; } // so that coulomb laws gives back the energy in kcalmol 
...
#elif defined (AMBER) || defined (PLUMED_CP2K) 
...
#ifdef __PLUMED_MPI
void PREFIX set_comm(MPI_Comm comm){
  mtd_data.comm=comm;
  mtd_data.intercomm=MPI_COMM_NULL;
}
#endif
void PREFIX mtd_data_init(int atoms,real ddt,real *mass,real *charge,real myboltz,char *metain){
 mtd_data.pos=float_2d_array_alloc(atoms,3);
 mtd_data.vel=float_2d_array_alloc(atoms,3);
 mtd_data.force=float_2d_array_alloc(atoms,3);
 mtd_data.charge=(real *)calloc(atoms,sizeof(real));
 for (int i=0;i<atoms;i++){mtd_data.charge[i]= charge[i]; }
 mtd_data.mass=(real *)calloc(atoms,sizeof(real));
 for (int i=0;i<atoms;i++){mtd_data.mass[i]=mass[i]; } 
 mtd_data.fplog = fopen("plumed.out","w"); 
 mtd_data.natoms = atoms;
 mtd_data.dt = ddt ; // timestep from ps to fs  
 mtd_data.repl  = -1;
 mtd_data.eunit = 1.;
 mtd_data.boltz   = myboltz;
 mtd_data.ionode = (plumed_comm_rank(&mtd_data)==0);
 strcpy(mtd_data.metaFilename,metain);
 sprintf(hills.dir, ".");
 logical.not_same_step=1;
 mtd_data.istep_old = -1;
 mtd_data.istep = 0;
};
void PREFIX meta_force_calculation(int istep, real*xxx,real*fff,real *energy){
 mtd_data.istep=istep;
  for(int i=0;i<mtd_data.natoms;i++){
   mtd_data.pos[i][0]=xxx[3*i+0];
   mtd_data.pos[i][1]=xxx[3*i+1];
   mtd_data.pos[i][2]=xxx[3*i+2];
 }
 ...
 // step increment
 for(int i=0;i<mtd_data.natoms;i++){
   fff[3*i+0]+=mtd_data.force[i][0];
   fff[3*i+1]+=mtd_data.force[i][1];
   fff[3*i+2]+=mtd_data.force[i][2];
 }
 *energy+=cvw.Vwall+hills.Vhills;
};
#endif
...
void PREFIX plumed_sum(struct mtd_data_s *mtd_data,int nr,real r[]){
  ...
  if (nr > mtd_data->sum_nalloc) {
    mtd_data->sum_nalloc = nr;
    srenew(mtd_data->sum_buf,mtd_data->sum_nalloc);
  }

  if(sizeof(real)==sizeof(double)){
    MPI_Allreduce(r,mtd_data->sum_buf,nr,MPI_DOUBLE,MPI_SUM,mtd_data->comm);
  } else if(sizeof(real)==sizeof(float)){
    MPI_Allreduce(r,mtd_data->sum_buf,nr,MPI_FLOAT,MPI_SUM,mtd_data->comm);
  } else assert(0);

  for(i=0; i<nr; i++)
    r[i] = mtd_data->sum_buf[i];
#endif
};
...
void PREFIX plumed_sumi(struct mtd_data_s *mtd_data,int nr,int r[]){
  ...
  if (nr > mtd_data->sumi_nalloc) {
    mtd_data->sumi_nalloc = nr;
    srenew(mtd_data->sumi_buf,mtd_data->sumi_nalloc);
  }

    MPI_Allreduce(r,mtd_data->sumi_buf,nr,MPI_INT,MPI_SUM,mtd_data->comm);

  for(i=0; i<nr; i++)
    r[i] = mtd_data->sumi_buf[i];
#endif
};
...
void PREFIX plumed_intersum(struct mtd_data_s *mtd_data,int nr,real r[]){
  ...
  if (nr > mtd_data->intersum_nalloc) {
    mtd_data->intersum_nalloc = nr;
    srenew(mtd_data->intersum_buf,mtd_data->intersum_nalloc);
  }

  if(sizeof(real)==sizeof(double)){
    MPI_Allreduce(r,mtd_data->intersum_buf,nr,MPI_DOUBLE,MPI_SUM,mtd_data->intercomm);
  } else if(sizeof(real)==sizeof(float)){
    MPI_Allreduce(r,mtd_data->intersum_buf,nr,MPI_FLOAT,MPI_SUM,mtd_data->intercomm);
  } else assert(0);

  for(i=0; i<nr; i++)
    r[i] = mtd_data->intersum_buf[i];
#endif
};
...
#elif defined (PLUMED_GENERIC_CXX)
void PREFIX init_metadyn(int atoms,real ddt,real *mass,real *charge,real myboltz,real factor_angstrom,char *metain)
#endif
{
  long int j;
  real k;
...
// initializing previously static data:
  mtd_data.hills_file=NULL;
  mtd_data.hills_push_first=1;
  mtd_data.hills_add_first=1;
  mtd_data.debug_grid_file=NULL;
  mtd_data.cv_file=NULL;
  mtd_data.sum_nalloc=0;
  mtd_data.sum_buf=NULL;
  mtd_data.sumi_nalloc=0;
  mtd_data.sumi_buf=NULL;
  mtd_data.intersum_nalloc=0;
  mtd_data.intersum_buf=NULL;
  mtd_data.puckering_R=NULL;
  mtd_data.testderivatives_first=1;
  mtd_data.testderivatives_pos=NULL;
  mtd_data.testderivatives_der=NULL;
  mtd_data.testderivatives_previous_pos=NULL;
  mtd_data.testderivatives_previous_der=NULL;
  mtd_data.testderivatives_integral=NULL;
  mtd_data.testderivatives_debug_file=NULL;
  mtd_data.fplog=NULL;

#if defined (PLUMED_GROMACS)
#if defined (GMX_THREADS) || defined (GMX_THREAD_MPI)
...
#elif PLUMED_GENERIC_CXX
   mtd_data.factor_angstrom=factor_angstrom;
   mtd_data_init(atoms,ddt,mass,charge,myboltz,metain);
#endif
...
// restore printout
  ...
  if(logical.do_probres==1){
    if(logical.welltemp==1 || logical.metadymer==1) {
      plumed_error("MM-META AND WELLTEMPERED ARE NOT COMPATIBLE WITH PROBABILITY RESTRAINTS");
    }
    if(logical.do_grid==1){
      fprintf(stderr, "PLUMED ERROR: PROBABILITY RESTRAINTS DOES NOT SUPPORT GRID YET, OPTION MUST BE TURNED OFF!\n");
      logical.do_grid=0;
      exit(1);
    }
    int countp,countv;
    real probres_tot_val,probres_av_val,probres_std_val;
    for(countv=0;countv<colvar.nconst;countv++) {     
       if(colvar.on[countv]) {
         if (logical.probres[countv]) {
           hills.count_pres[countv]=0;
           hills.step_pres[countv]=(hills.probres_cv[hills.probrespoints[countv]-1][countv]-hills.probres_cv[0][countv])/(hills.probrespoints[countv]-1);
           hills.count_tot_pres[countv]=0;
           hills.count_tot_pres_hills[countv]=0;
           hills.cv_tot_pres_half[countv]=0;
           hills.cv_tot_pres[countv]=0; 
           probres_tot_val=0; 
           hills.cvsq_tot_pres[countv]=0;
           hills.cvsq_tot_pres_half[countv]=0; 
           if(logical.do_inversion){
             fprintf(stderr, "PLUMED REMARK: IVERSION KEYWORD WITH PROBABILITY RESTRAINTS WILL JUST DO REFLECTION; USE IT WITH INTERVAL! \n");
//             logical.do_inversion=0;
//             exit(1); 
           }
           // normalize probability density
           for(countp=0;countp<hills.probrespoints[countv]-1;countp++) {
              probres_tot_val+=0.5*(hills.probres_val[countp+1][countv]+hills.probres_val[countp][countv])*fabs(hills.probres_cv[countp+1][countv]-hills.probres_cv[countp][countv]);
              if(hills.probres_val[countp+1][countv]<=0||hills.probres_val[countp][countv]<=0){
                fprintf(stderr, "PLUMED ERROR: PROBABILITY MUST BE LARGER THAN 0! \n"); 
                exit(1); 
              }
           }
           for(countp=0;countp<hills.probrespoints[countv];countp++) hills.probres_val[countp][countv]=hills.probres_val[countp][countv]/probres_tot_val; // probability density normalized
           // calculate average cv and standard deviation
           probres_av_val=0;
           probres_std_val=0;
           for(countp=0;countp<hills.probrespoints[countv]-1;countp++) {
              probres_av_val+=0.5*((hills.probres_val[countp+1][countv]*hills.probres_cv[countp+1][countv])+(hills.probres_val[countp][countv]*hills.probres_cv[countp][countv]))*fabs(hills.probres_cv[countp+1][countv]-hills.probres_cv[countp][countv]);
              probres_std_val+=0.5*((hills.probres_val[countp+1][countv]*(pow(hills.probres_cv[countp+1][countv],2)))+(hills.probres_val[countp][countv]*(pow(hills.probres_cv[countp][countv],2))))*fabs(hills.probres_cv[countp+1][countv]-hills.probres_cv[countp][countv]);
           }
           hills.av_cv_pres[countv]=probres_av_val;
           hills.std_cv_pres[countv]=sqrt(probres_std_val-(pow(probres_av_val,2)));
           // calculate differential entropy 
           probres_tot_val=0;
           for(countp=0;countp<hills.probrespoints[countv]-1;countp++) {
              probres_tot_val+=0.5*((hills.probres_val[countp+1][countv]*log(hills.probres_val[countp+1][countv]))+(hills.probres_val[countp][countv]*log(hills.probres_val[countp][countv])))*fabs(hills.probres_cv[countp+1][countv]-hills.probres_cv[countp][countv]);
              // negative of differential entropy calculated
           }
           for(countp=0;countp<hills.probrespoints[countv];countp++) hills.probres_val[countp][countv]=hills.probres_val[countp][countv]*exp(-probres_tot_val); 
           // probability density multiplied by the effective volume = exp(differential entropy)
           fprintf(mtd_data.fplog, " PROBRES, CV %i: AVERAGE %e, STANDARD DEVIATION %e, EFFECTIVE VOLUME %e  \n",countv+1,hills.av_cv_pres[countv],hills.std_cv_pres[countv],exp(-probres_tot_val));
         }
       }

       if(logical.interval[countv]) {
         if(cvint.lower_limit[countv]<hills.probres_cv[0][countv] || cvint.upper_limit[countv] > hills.probres_cv[hills.probrespoints[countv]-1][countv]){
           fprintf(stderr, "PLUMED ERROR: INTERVALS LIMITS MUST BE INSIDE PROBRES LIMITS!\n");
           exit(1);
         }
       }

    }
//    cvint.lower_limit[icv]>hills.probres_cv[0][icv] && cvint.upper_limit[icv]<hills.probres_cv[hills.probrespoints[icv]-1][icv]
  }
...
#endif
  *mod_rij=sqrt(pow(rij[0],2)+pow(rij[1],2)+pow(rij[2],2));
#elif defined (ACEMD) || defined (DRIVER) || defined (AMBER) || defined (STANDALONE) || defined (PLUMED_QESPRESSO) || defined (PLUMED_CP2K)   
  int i;
...
void PREFIX mm_meta_init(){
    int i,ii,jj, nhills;
//    fprintf(stderr, "MM-META INIT PIPPO \n");
    colvar.nconst_active=0;
    if(logical.do_grid==1){
      fprintf(stderr, "PLUMED ERROR: MM-META DOES NOT SUPPORT GRID, OPTION MUST BE TURNED OFF!\n");
      logical.do_grid=0;
      exit(1);
    }
//    if(colvar.nt_medy/hills.nt_hills!=floor(colvar.nt_medy/hills.nt_hills)) plumed_error("WITH MM-META \"AVER\" MUST BE A MULTIPLE OF \"W_STRIDE\"");
//    if(colvar.nt_medy<hills.nt_hills) plumed_error("WITH MM-META \"AVER\" MUST BE LARGER THAN \"W_STRIDE\"");
    for(ii=0;ii<colvar.nconst;++ii){
       if(colvar.on[ii]){
         colvar.iactive[colvar.nconst_active]=ii;
         colvar.nconst_active++;
       }
    }
    // allocate and initialize variables
    colvar.medy2          = float_1d_array_alloc(colvar.nconst_active);
    colvar.medy_ver       = float_1d_array_alloc(colvar.nconst_active);
    colvar.medy           = float_2d_array_alloc(colvar.nconst_active,colvar.nt_medy);
    if(logical.dymerlocal==1) {
      colvar.medy_ver_loc   = float_3d_array_alloc(colvar.ndim_medy,colvar.nconst_active,colvar.nconst_active);
      colvar.count_dim_medy = int_1d_array_alloc(colvar.ndim_medy);
      colvar.vercomp        = int_2d_array_alloc(colvar.ndim_medy,colvar.ndim_medy);
      for(i=0;i<colvar.ndim_medy;++i) {
         colvar.count_dim_medy[i]=0;
         for(ii=0;ii<colvar.nconst_active;++ii) {
            for(jj=0;jj<colvar.nconst_active;++jj) {
               colvar.medy_ver_loc[i][ii][jj]=0;
            }
         }
      }
    }
    ...
    if(logical.thr_dy==1) {
      ...
      colvar.wfact_loc_medy_tmp=colvar.wfact_loc_medy;
    } 
    for(i=0;i<colvar.nconst_active;++i){
      ii=colvar.iactive[i]; 
      colvar.medy2[i]=0.;
      colvar.medy_ver[i]=0;
      if(logical.interval[ii]==1) {
        logical.interval[ii]=0;
        fprintf(stderr, "PLUMED ERROR: MM-META DOES NOT SUPPORT INTERVAL, OPTION MUST BE TURNED OFF!\n");
        exit(1);
      }
    }
    //------------------------------------------ MM-META HILLS FILES --------------------------- 
    if(logical.do_hills){
      nhills = STACKDIM ;// dimesion of hills vector
      if(logical.dymerlocal==0) {
        // dimension of hills vector (NONLOCAL)->MM-META
        hills.medy_ver_nh     = float_2d_array_alloc(nhills,colvar.nconst_active);
        // END dimesion of hills vector (NONLOCAL)->MM-META
        for (i=0;i<nhills;++i) {
           for (ii=0;ii<colvar.nconst_active;++ii) {
              hills.medy_ver_nh[i][ii]=0;
           }
        }
      }
      if(logical.dymerlocal==1) {
        // dimension of hills vector->MM-META
        hills.ww_wfact        = float_1d_array_alloc(nhills);
        hills.medy_ver_loc_nh = float_3d_array_alloc(nhills,colvar.nconst_active,colvar.nconst_active);
        // END dimesion of hills vector->MM-META
        for (i=0;i<nhills;++i) {
           for (ii=0;ii<colvar.nconst_active;++ii) {
              for (jj=0;jj<colvar.nconst_active;++jj) {
                 hills.medy_ver_loc_nh[i][ii][jj]=0;
              }
           }
        }
      }
    }
    //------------------------------------------ MM-META HILLS FILES END -----------------------
}
